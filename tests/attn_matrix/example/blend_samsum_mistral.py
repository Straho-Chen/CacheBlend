...
from utils.utils import REPO_ROOT, load_dataset, build_fewshot_prompt_normal, compute_rl, export_attention_matrices
...
test_model="/workspaces/cjh/model/Mistral-7B-Instruct-v0.2/"
...
for sample_idx, ex in enumerate(eval_dataset):
    ...
    print(f"len of doc_chunk_ids before dropping: {len(doc_chunk_ids)}")
    ...
    print(f"len of doc_chunk_ids after dropping: {len(doc_chunk_ids)}")
    ...
    print(f"len of doc_chunk_ids:", len(doc_chunk_ids))
    ...
    if args.use_cache:
        ...
        for i in range(len(doc_chunk_ids)):
            ...
            export_attention_matrices(cache_fuse_metadata, name_prefix=f"chunk{i}_", export_dir="./attn_exports_mistral")
        ...
        export_attention_matrices(cache_fuse_metadata, name_prefix=f"prefill_", export_dir="./attn_exports_mistral")
