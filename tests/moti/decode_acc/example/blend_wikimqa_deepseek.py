...
from utils.utils import REPO_ROOT, load_dataset, build_qa_prompt_normal, compute_f1, extract_after_think, export_last_layer_hidden_states
import argparse
...
for ex in eval_dataset:
    ...
    cache_fuse_metadata['export_last_layer_hidden_states'] = True
    # print("Generating with cache-fuse blending...")
    ...
    for answer in answers:
        f1, precision, recall = compute_f1(res, answer[0], tokenizer)
        f1_max = max(f1_max, f1)
        precision_max = max(precision_max, precision)
        recall_max = max(recall_max, recall)
    cache_fuse_metadata["f1"] = f1_max
    cache_fuse_metadata["precision"] = precision_max
    cache_fuse_metadata["recall"] = recall_max
    # export_last_layer_hidden_states(cache_fuse_metadata, f"blend_{sample}_")
    ...
    cache_fuse_metadata["precision"] = precision_max
    cache_fuse_metadata["recall"] = recall_max
    # export_last_layer_hidden_states(cache_fuse_metadata, f"full_reuse_{sample}_")
    ...
    cache_fuse_metadata["precision"] = precision_max
    cache_fuse_metadata["recall"] = recall_max
    # export_last_layer_hidden_states(cache_fuse_metadata, f"full_prefill_{sample}_")
