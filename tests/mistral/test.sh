...
#         python example/blend_${DATASET_NAME}_mistral.py --cache --recomp-ratio $recomp_ratio > $log_file 2>&1
...
for DATASET_NAME in ${DATASET[@]}; do
    echo "Testing $DATASET_NAME..."
    log_file=$OUTPUT_DIR/blend_${DATASET_NAME}_mistral.txt
    python example/blend_${DATASET_NAME}_mistral.py > $log_file 2>&1
